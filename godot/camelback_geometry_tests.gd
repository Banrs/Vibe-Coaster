...
func _test_seed_42_camelback_is_a_reviewed_planar_element() -> void:
	...
	_t.expect(record.get("status") == "adopted",
	...
	_t.expect(_finite_at_most(measurement, "vertical_plane_tilt_deg",
	...
	_t.expect(_finite_at_most(measurement, "out_of_plane_ratio",
	...
	_t.expect(_finite_at_most(measurement, "heading_drift_deg",
	...
	_t.expect(_finite_abs_at_most(measurement, "entry_bank_deg", MAX_BOUNDARY_BANK_DEG),
		"camelback enters near neutral bank; got %s degrees"
		% str(measurement.get("entry_bank_deg")))
	_t.expect(_finite_abs_at_most(measurement, "exit_bank_deg", MAX_BOUNDARY_BANK_DEG),
