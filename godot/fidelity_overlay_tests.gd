...
func _initialize() -> void:
	if not ResourceLoader.exists(OVERLAY_PATH):
		_errors.append("RideFidelityOverlay is missing")
	else:
...
func _test_valid_semantic_overlay(overlay: Script) -> void:
	...
	_expect(result.get("status") == "ok", "valid bytes produce an overlay: %s" % str(result))
	if result.get("status") != "ok": return
	_expect(result.get("schema_version") == "fidelity-semantic-overlays@1",
		"overlay declares its schema")
	_expect(result.recordings[0].validation_status == "available"
	...
	_expect(comparison.alignment_status == "semantic_only", "alignment is semantic only")
	_expect(comparison.source.window_s == [0.0, 0.06]
		and comparison.source.sample_count == 3,
		"source selection is native and half-open")
	_expect(comparison.generated.window_s == [20.0, 21.0]
	...
	_expect(row_two.window_s == [20.0, 21.0] and row_two.sample_count == 2
	...
	_expect(_times(source.samples) == [0.0, 0.02, 0.04]
	...
	_expect(is_equal_approx(generated.samples[0].normal_g, row_four_series.normal_g[1])
	...
	_expect(not is_equal_approx(generated.samples[0].normal_g, row_two_lane.samples[0].normal_g),
		"changing the selected modeled row changes generated force values")
	_expect(not source.samples[1].eligible and source.samples[0].eligible,
		"masked samples remain present and ineligible")
	_expect(is_equal_approx(target.samples[0].normal_g, 2.0 * 1.3333333333)
		and is_equal_approx(target.samples[1].normal_g, -1.5),
		"normal transforms are sign-aware")
	_expect(is_equal_approx(target.samples[0].lateral_g, 1.5666666667)
		and is_equal_approx(target.samples[1].lateral_g, -2.0 * 1.5666666667),
		"lateral transforms preserve sign")
	_expect(is_equal_approx(target.samples[0].longitudinal_g, -1.7142857143)
		and target.samples[1].longitudinal_g == null,
		"negative longitudinal is scaled while positive longitudinal is a gap")
	_expect(is_equal_approx(generated.samples[1].normal_g, row_four_series.normal_g[2])
	...
	_expect(is_equal_approx(source.summary.axes.normal_g.denominator_s, 0.04)
	...
	_expect(is_equal_approx(generated.summary.axes.normal_g.denominator_s, 1.0)
	...
	_expect(is_equal_approx(target.summary.axes.normal_g.denominator_s, 0.04)
		...
		"scaled target reports each axis against its own retained denominator: %s" \
			% str(target.summary))
	_expect(comparison.markers.size() == 3
	...
	_expect(JSON.stringify(result) == JSON.stringify(repeat),
		"dictionary insertion order cannot change the deterministic result")
	_expect(not JSON.stringify(result).contains(path), "local absolute paths are never serialized")
	for comparison_key in comparison.keys():
		_expect(str(comparison_key) not in ["aligned_time", "normalized_time", "common_time"],
...
func _test_exact_anchor_resolution(overlay: Script) -> void:
	...
	for beats in [[], [_measurement().beats[0], _measurement().beats[0].duplicate(true)]]:
		...
		var comparison: Dictionary = result.comparisons[0]
		_expect(comparison.generated.status == "generated_window_unavailable"
			and _lane(comparison, "generated_raw").samples.is_empty(),
			"zero or duplicate exact anchors do not use fuzzy fallback")
		_expect(_lane(comparison, "source_observed_smoothed").samples.size() == 3,
...
func _test_manifest_validation(overlay: Script) -> void:
	for manifest in [{}, {"schema_version": "wrong", "recordings": []},
		...
		_expect(result.get("status") == "invalid-input" and not result.get("errors", []).is_empty(),
	...
	_expect(_build(overlay, path_shaped, {}, _measurement(), _route()).status == "invalid-input",
	...
	for malformed: Dictionary in cases:
		...
		_expect(result.status == "invalid-input" and not result.errors.is_empty(),
...
func _test_exact_manifest_digest(overlay: Script) -> void:
	...
	_expect(result.manifest_sha256 == FileAccess.get_sha256(MANIFEST_PATH),
	...
	if different_bytes != bytes:
		_expect(overlay.build(manifest, different_bytes, {}, _measurement(), _route(),
	...
	_expect(overlay.build({}, mismatched.to_utf8_buffer(), {}, _measurement(), _route(),
...
func _test_transform_authority(overlay: Script) -> void:
	...
	_expect(_lane(result.comparisons[0], "approved_scaled_target").samples[0].normal_g == 4.0,
	...
	_expect(overlay.build(manifest, bytes, {}, _measurement(), _route(),
...
func _test_committed_manifest(overlay: Script) -> void:
	...
	_expect(manifest is Dictionary, "committed overlay manifest reopens as JSON")
	...
	_expect(result.status == "ok" and result.recordings.size() == 2
	...
	_expect(unavailable.is_empty() and anchored.get("comparisons", []).size() == 4,
...
func _test_optional_local_exports(overlay: Script) -> void:
	...
	_expect(result.recordings.map(func(record: Dictionary): return record.validation_status)
		== ["available", "available"],
	...
	_expect(counts == [338, 115, 165, 232],
