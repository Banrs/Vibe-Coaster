...
static func composite(
	...
	canvas.fill_rect(Rect2i(PANE_SIZE.x, 0, DIVIDER, PANE_SIZE.y), DIVIDER_COLOR)
	canvas.fill_rect(Rect2i(0, PANE_SIZE.y, width, FOOTER_HEIGHT), FOOTER_BACKGROUND)
...
# ---------------------------------------------------------------------------------------------
# A 5x7 bitmap font, one packed string of seven row patterns per glyph. The repository has no font
# resource and no other module draws text, so the glyphs are literal pixel rows. Unknown
...
## One glyph as scale-sized blocks. Image.fill_rect clips to the canvas, so no bounds test is needed.
static func _draw_glyph(
	image: Image, pattern: String, origin: Vector2i, scale: int, color: Color
) -> void:
	var rows := pattern.split(" ")
	for row in rows.size():
		var bits: String = rows[row]
		for column in bits.length():
			if bits[column] == "1":
				image.fill_rect(
					Rect2i(origin + Vector2i(column, row) * scale, Vector2i(scale, scale)), color)
