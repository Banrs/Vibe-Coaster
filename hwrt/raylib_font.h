// AUTO-GENERATED from src/vendor/raylib/src/rtext.c — raylib default font (pixel-exact).
// 128x128 1-bit atlas (512 uints, 32 px/uint), 224 glyphs from codepoint 32, 10px tall.
#pragma once
static const unsigned int RL_FONT_DATA[512] = {
...
static const int RL_CHAR_W[224] = {
...
// ---------------------------------------------------------------------------
// Core Graphics renderer for raylib's default font (pixel-exact 1:1 with the
// software game's DrawText). Builds the 128x128 1-bit atlas into a CGImage mask
// once, reconstructs glyph rects exactly as rtext.c does, and lays text out with
// raylib's DrawTextEx math (scaleFactor = fontSize/10, spacing = fontSize/10).
// Included only by main.mm (needs CoreGraphics/AppKit).
// ---------------------------------------------------------------------------
#ifdef __OBJC__
...
// width of a string at fontSize, matching raylib MeasureText (default font).
static CGFloat rlMeasureText(const char* s, CGFloat fontSize) {
    if (fontSize < 10) fontSize = 10;
    CGFloat scale = fontSize / 10.0, spacing = floorf(fontSize / 10.0);
    const RLGlyph* g = rlGlyphs();
    CGFloat w = 0;
    for (const char* p = s; *p; p++) {
        int idx = (unsigned char)*p - 32; if (idx < 0 || idx >= 224) idx = 0;
        w += g[idx].w * scale + spacing;
    }
...
// Shadowed text, matching the SW game's textSh (dark drop shadow then colour).
static void rlText(CGContextRef c, const char* s, CGFloat x, CGFloat y, CGFloat sz,
                   CGFloat r, CGFloat g_, CGFloat b, CGFloat a) {
    rlDrawText(c, s, x + 2, y + 2, sz, 0.08, 0.08, 0.12, 0.78 * a);
    rlDrawText(c, s, x, y, sz, r, g_, b, a);
}
#endif
